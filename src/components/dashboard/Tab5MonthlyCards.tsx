...
export const Tab5MonthlyCards = ({
  records,
  quadrimestre = "todos",
  mesReferencia = [],
  equipe = "all",
  oficialData,
}: Tab5MonthlyCardsProps) => {
  ...
  const monthlyData = useMemo(() => {
    ...
    return Array.from(byMonth.entries())
      .map(([mesAno, prel]) => {
        // ── Merge oficial vs preliminar (B5) ──────────────────────────────
        ...
        const percentage = total > 0 ? (preventivos / total) * 100 : 0;
        return { mesAno, preventivos, total, percentage, fonte };
      })
      .sort((a, b) => {
