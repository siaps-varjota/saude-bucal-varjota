...
export const Tab3MonthlyCards = ({ patients }: Tab3MonthlyCardsProps) => {
  const monthlyData = useMemo(() => {
    const now = new Date();
    const months: { month: number; year: number; count: number }[] = [];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 text-xs">
        ...
        <Badge className="bg-red-500 hover:bg-red-500 text-white">Regular: ≤ 5</Badge>
        <Badge className="bg-amber-500 hover:bg-amber-500 text-white">Suficiente: 6-10</Badge>
        <Badge className="bg-emerald-500 hover:bg-emerald-500 text-white">Bom: 11-15</Badge>
        <Badge className="bg-blue-500 hover:bg-blue-500 text-white">Ótimo: &gt; 15</Badge>
      </div>
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-12">
        {monthlyData.map((data, index) => {
          const getVariant = (count: number) => {
            if (count <= 5) return "bg-red-500/10 border-red-500/30 text-red-700";
            if (count <= 10) return "bg-amber-500/10 border-amber-500/30 text-amber-700";
            if (count <= 15) return "bg-emerald-500/10 border-emerald-500/30 text-emerald-700";
          ...
          return (
            <Card 
              key={index} 
              className={`border ${getVariant(data.count)} transition-all hover:scale-105`}
            >
