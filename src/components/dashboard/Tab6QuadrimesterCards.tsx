...
export const Tab6QuadrimesterCards = ({
  ...
  const quadCounts = useMemo(() => {
    ...
    return quadrimesters.map((q) => {
      ...
      return { ...q, totalExodontias, totalProcedimentos, percentage, avgMonthlyExodontias, monthsWithData, mesesLista, fonte };
    });
  ...
  return (
    <>
      {visibleCards.map((quad) => {
        ...
        return (
          <Card key={quad.label} className={`border-0 shadow-md transition-all hover:shadow-lg hover:-translate-y-0.5 h-full ${styles.bg}`}>
            <CardContent className="p-4 flex flex-col justify-center h-full">
