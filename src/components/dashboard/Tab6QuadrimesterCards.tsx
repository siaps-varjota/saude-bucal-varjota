...
export const Tab6QuadrimesterCards = ({
  ...
  const quadCounts = useMemo(() => {
    ...
    return quadrimesters.map(q => {
      ...
      const avgPercentage = monthlyPercentages.length > 0 ? monthlyPercentages.reduce((s, p) => s + p, 0) / monthlyPercentages.length : 0;
      const avgMonthlySim = monthlyData.length > 0 ? countSim / monthlyData.length : 0;
      return {
        ...q,
        countSim,
        totalRecords,
  ...
  return <>
      ...
      return <Card key={quad.label} className={`border-0 shadow-md transition-all hover:shadow-lg ${styles.bg}`}>
